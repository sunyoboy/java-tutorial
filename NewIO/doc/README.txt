...
=========
    ...
    % java -classpath build org.lieve.nio.chatserver.ChatServer [-port <port number>]

    Usage:  org.lieve.nio.chatserver.ChatServer [options]
        options:
...
====================
org.lieve.nio.chatserver.ChatServer is the main class, it handles the startup and handles incoming
...
org.lieve.nio.chatserver.Client represents a connected user, it provides methods for reading/writing
from/to the underlying socket. It also contains a buffer of input read from
the user.

org.lieve.nio.chatserver.DataReader provides the interface of the two states a user can
be in. Waiting for a name (and not receiving any messages while doing so, implemented
by org.lieve.nio.chatserver.NameReader) and waiting for messages from the user (implemented by org.lieve.nio.chatserver.MessageReader).

org.lieve.nio.chatserver.ClientReader contains the "main loop" for a connected client.

org.lieve.nio.chatserver.NameReader is the initial state for a new client, it sends the user a string and
waits for a response before changing the state to org.lieve.nio.chatserver.MessageReader.

org.lieve.nio.chatserver.MessageReader is the main state for a client, it checks for new messages to send to
